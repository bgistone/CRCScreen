...
def help():
	...
	-s, --saturation	<int>	increment of reads sampling for saturation analysis[0]
	-c, --cutoff	<int>	depth cutoff for genotyping reliability [20]
	-h, --help	call help
	'''
	exit()

opts, args = getopt.getopt(sys.argv[1:], 'hf:r:o:s:c:', ['read1=', 'read2=', 'output=', 'saturation', 'cutoff'])
...
if saturation != 0:
	saturationTest = open(output+".saturation.txt","w")
	saturationTest.write("Reads\tCross_Matched\tNormal\tPathogenic\tuncountable")
	for k in aa:
		saturationTest.write("\t"+k)
	saturationTest.write("\n")
#================core===============
class Core:
		...
		with open(read1, 'r') as finA, \
				open(read2, 'r') as finB:
					...
					for r1, r2 in izip(SeqIO.parse(finA, "fastq"), SeqIO.parse(finB, "fastq")):
						...
						if saturation != 0 and Total_Reads%saturation == 0:
							saturationTest.write(str(Total_Reads))
							saturationTest.write("\t" + str(Cross_Matched_Reads))
							saturationTest.write("\t" + str(toolBox.Normal_Reads))
							saturationTest.write("\t" + str(float(toolBox.Pathogenic_Reads)/Cross_Matched_Reads*100))
							saturationTest.write("\t" + str(float(toolBox.unKnown_Reads)/Cross_Matched_Reads*100))
							for vk in aa:
								if vk in disease_ratio:
									ratio = float(disease_ratio[vk]) / Cross_Matched_Reads * 100
									saturationTest.write("\t" + str(ratio))
								else:
									saturationTest.write("\t0.00")
							saturationTest.write("\n")
